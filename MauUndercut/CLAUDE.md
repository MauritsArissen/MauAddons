...
- Searches: commodities `SendSearchQuery(MakeItemKey(itemID), sorts Price asc, true)`; equipment `SendSellSearchQuery(MakeItemKey(itemID), sorts Buyout asc, true)` (item level and suffix must be cleared for sell searches, results carry the real keys); other items `SendSearchQuery(full key, ...)`. The key actually sent is `entry.searchKey`, computed in `BeginSearch`.
- **Cached key info is required for the key that is sent** (0.7.1 fix). The client silently ignores a search for an item key whose `GetItemKeyInfo` is not cached yet (documented in Auctionator's `AH/Mixins/SearchScan.lua`; Blizzard's `AuctionHouseItemSellFrameMixin:SetItem` calls `GetItemKeyInfo` on the cleared key before `QueryItem`). Until 0.7.0 only the bag item's full key was checked, so the first sell search for a piece of gear with a random suffix (full key cached, cleared key not) got no answer, hit the 10 s timeout, and `SearchFailed` fell straight back to the remembered price or vendor × 2. Now `BeginSearch` waits (0.5 s polls, `ITEM_KEY_ITEM_INFO_RECEIVED`) until `GetItemKeyInfo(entry.searchKey)` returns, and a timeout re-sends the query (`RetrySearch`) up to `MAX_SEARCH_ATTEMPTS` before giving up; `SEARCH_TIMEOUT` is 6 s.
- Results arrive on `COMMODITY_SEARCH_RESULTS_UPDATED(itemID)` / `ITEM_SEARCH_RESULTS_UPDATED(itemKey)`. For equipment the event key is matched on `itemID` only (the search is by item ID). The Auctionator workaround is used: if `HasSearchResults` is false, or results are not full with zero quantity, retry (max 4 attempts). For items, more pages are requested while no exact variant is loaded (max 5); `RequestMoreItemSearchResults` returning true means everything is already loaded and no event will follow, so the results at hand are used at once.
