...
| `MauGuildMap.xml` | `MauGuildMapPinTemplate`: a 22×22 Frame with a 22×22 class-coloured `Ring` (white texture clipped by `RingMask`) under a 20×20 `Icon` texture clipped round by `CircleMask`, `enableMouseMotion="true"`, `mixin="MauGuildMapPinMixin"`. |
...
- Position message: `2:<token>:<mapID>:<x>:<y>:<level>:<raceFile>:<sex>:<classFile>:<flag>:<instanceName>`; `token` is a random hex id chosen at login (`Comm.token`; test members use `0`), x/y are map coordinates × 10000; `flag` is `O` (outdoors) or `I` (in an instance: position = entrance); the instance name is last so it may contain anything. `B` alone = goodbye. Typical length 50–65 bytes (limit 255). Version 1 (no token) is rejected by the decoder.
