...
| `detailedTooltip` | false | long tooltip with numbers instead of the three-line one |
...
- Per-pin settings (0.2.0) are applied in `OnAcquired`, which runs on every refresh: `SetScalingLimits(1, pinScale, pinScale)` then `ApplyCurrentScale()`, `Ring:SetShown(ring)`, `Label` (name in class colour, `labels`), `Skull:SetShown(dead and deathMarkers)` with the icon desaturated. Tooltip (0.2.1, after the user found the first version cluttered): compact by default, three lines: name with the level right-aligned, race and class with the zone (or "Instance (entrance)") right-aligned, and one line with "Dead", "HP 62%", "Mana 40%", "XP 31%" coloured, only for values the entry has and the viewer's `show*` settings allow; "No update for N s" appears only after 30 s. `detailedTooltip` (default false) restores the long form with the actual numbers, "N to level L+1", zone details and the update age. It runs on map change/show (Blizzard), on every roster change while the map is shown (`Map:RequestRefresh`, coalesced to 0.25 s) and every 5 s while shown (ages, timeouts).
...
- Test entries carry `entry.test = true` (shown as "(test)" in the tooltip and `/mgm list`). Nothing is printed when members come or go; the test mode announces its timeline once at the start (0.2.1, chat clutter).
