...
| `iconSize` | 20 | race icon size in px (12–40) |
| `iconZoom` | 30 | percent of the icon cropped away to hide its rim (0–50) |
| `instanceAlpha` | 80 | opacity in percent for members inside an instance (20–100) |
| `ring` / `ringWidth` | true / 1 | class-coloured ring and its width in px (1–4) |
| `labels` | false | name labels |
| `labelFont` | "friz" | `NS.FONTS` key: friz, arial, skurri, morpheus (files every client ships) |
| `labelSize` | 10 | label size in points (6–20) |
| `labelOutline` | "none" | `NS.OUTLINES` key: none (drop shadow), outline, thick |
| `labelPosition` | "below" | below or above the icon |
| `labelOffset` | 1 | px between icon and label (0–12) |
| `labelClassColor` | true | class colour, else white |
| `showHealth` / `showPower` / `showXP` | true | what the tooltip shows (of what the other side shares) |

Numeric ranges live in `NS.RANGES` (min, max, slider step) and are clamped in `InitDB`; `pinScale` from 0.2.x is deleted there. Sizes are applied per pin in `OnAcquired`: frame and ring = `iconSize + 2 × ringWidth`, skull = `iconSize − 2`, label via `FontString:SetFont(path, size, flags)` with a fallback to the default font if the path fails, anchored above or below by `labelOffset`. Dropdowns use `Settings.CreateDropdown` with a `Settings.CreateControlTextContainer` (string values, `Settings.VarType.String`).
...
- Every `SEND_INTERVAL` (2 s) `Comm:Tick` samples the state and sends when something changed (see "Change detection" below), or when `HEARTBEAT` (20 s) has passed. `PLAYER_LOGOUT` and switching "Send my position" off send `B`. Not sent when not in a guild or `settings.broadcast` is false.
- Guild membership (0.3.1): `Comm:CheckGuild` compares `IsInGuild()` with the last known state on `PLAYER_GUILD_UPDATE`, `GUILD_ROSTER_UPDATE` and every tick. Joining → send at once; leaving → `Roster:Clear()`. A first message from an unknown real member calls `Comm:AnnounceSoon` (one send after 0.5–2 s of jitter, coalesced), so a newcomer sees everyone within a couple of seconds instead of waiting for their heartbeats; test members do not trigger it. Reported 2026-09-26: a new guild member "did not start receiving or sending" (nothing is gated on the guild at load; the delay was heartbeats plus the tick).
...
- Per-pin settings are applied in `OnAcquired`, which runs on every refresh (sizes, zoom, ring, opacity, label font/size/outline/position/colour, skull); see the settings table in section 2. `SetScalingLimits(1, 1, 1)` keeps the pixel sizes constant at every zoom level. Tooltip (settled in 0.2.2): the 0.1.1 layout, i.e. name, "Level N Race Class", "Dead" when dead, zone (or "In X" + "Shown at the entrance in Y"), "Updated N s ago", plus exactly one extra line for the stats: "HP 62%   Mana 40%   XP 31%" coloured, only for values the entry has and the viewer's `show*` settings allow. The user rejected both a one-line-per-stat version with numbers (0.2.0, too much text) and a compressed three-line tooltip (0.2.1, too far the other way); keep this shape. It runs on map change/show (Blizzard), on every roster change while the map is shown (`Map:RequestRefresh`, coalesced to 0.25 s) and every 5 s while shown (ages, timeouts).
...
- Test entries carry `entry.test = true` (shown in the tooltip and `/mgm list`); removal reasons are printed only for those.
